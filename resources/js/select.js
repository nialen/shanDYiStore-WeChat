...
(function() {
		...
		function(
			...
			return {
				...
				templateUrl: function(tElement) {
					...
					var theme =
						tElement.parent().attr('theme') || uiSelectConfig.theme;
				...
				compile: function(tElement, tAttrs) {
					...
					if (groupByExp) {
						var groups = tElement.querySelectorAll(
						...
						groups.attr(
							'ng-repeat',
							RepeatParser.getGroupNgRepeatExpression()
						);
					}

					var parserResult = RepeatParser.parse(tAttrs.repeat);

					var choices = tElement.querySelectorAll(
					...
					choices
						.attr(
							'ng-repeat',
							parserResult.repeatExpression(groupByExp)
						)
						.attr('ng-if', '$select.open'); //Prevent unnecessary watches when dropdown is closed

					var rowsInner = tElement.querySelectorAll(
					...
					var clickTarget = $window.document.addEventListener
						? choices
						: rowsInner;
					clickTarget.attr(
						'ng-click',
						'$select.select(' +
							parserResult.itemName +
							',$select.skipFocusser,$event)'
					);

					return function link(scope, element, attrs, $select) {
						$select.parseRepeatAttr(
							attrs.repeat,
							groupByExp,
							groupFilterExp
						...
						scope.$watch('$select.search', function(newValue) {
							...
							$select.activeIndex = $select.tagging.isActivated
								? -1
								: 0;
							if (
	...
	uis.controller('uiSelectCtrl', [
		...
		function(
			...
			ctrl.searchInput = $element.querySelectorAll(
			...
			ctrl.isEmpty = function() {
				return (
					angular.isUndefined(ctrl.selected) ||
					ctrl.selected === null ||
					ctrl.selected === '' ||
			...
			ctrl.activate = function(initSearchValue, avoidReset) {
				if (!ctrl.disabled && !ctrl.open) {
					...
					if (
						ctrl.activeIndex === -1 &&
						ctrl.taggingLabel !== false
					) {
						ctrl.activeIndex = 0;
					}

					var container = $element.querySelectorAll(
						'.ui-select-choices-content'
					);
					var searchInput = $element.querySelectorAll(
						'.ui-select-search'
					);
					if (
						ctrl.$animate &&
			...
			ctrl.parseRepeatAttr = function(
				...
				function updateGroups(items) {
					...
					angular.forEach(items, function(item) {
						var groupName = angular.isFunction(groupFn)
							? groupFn(item)
							: item[groupFn];
				...
				if (ctrl.parserResult.keyName) {
					...
					ctrl.parserResult.source = $parse(
						'$uisSource' + ctrl.parserResult.filters
					);
					$scope.$watch(
						originalSource,
				...
				$scope.$watchCollection(ctrl.parserResult.source, function(
					items
				) {
			...
			ctrl.refresh = function(refreshAttr) {
				if (refreshAttr !== undefined) {
					...
					_refreshDelayPromise = $timeout(function() {
						...
						if (
							refreshPromise &&
							angular.isFunction(refreshPromise.then) &&
							!ctrl.refreshing
						) {
			...
			ctrl.isActive = function(itemScope) {
				...
				var itemIndex = ctrl.items.indexOf(
					itemScope[ctrl.itemProperty]
				);
				...
				if (
					isActive &&
					!angular.isUndefined(ctrl.onHighlightCallback)
				) {
			...
			ctrl.isDisabled = function(itemScope) {
				...
				if (
					...
					if (
						...
						isDisabled = !!itemScope.$eval(
							ctrl.disableChoiceExpression
						);
					}
					...
					if (!item || !_isItemDisabled(item)) {
						...
						if (
							...
							if (ctrl.taggingLabel === false) {
								if (ctrl.activeIndex < 0) {
									if (item === undefined) {
										item =
											ctrl.tagging.fct !== undefined
												? ctrl.tagging.fct(ctrl.search)
												: ctrl.search;
									}
									if (
										!item ||
										angular.equals(ctrl.items[0], item)
									) {
										return;
									}
								} else {
						...
						$timeout(function() {
							ctrl.onSelectCallback($scope, {
								$item: item,
								$model: ctrl.parserResult.modelMapper(
									$scope,
									locals
								)
							});
			...
			$scope.$watch(function() {
				return (
					angular.isDefined(ctrl.lockChoiceExpression) &&
				...
				$scope.$apply(function() {
					...
					if (ctrl.items.length > 0 || ctrl.tagging.isActivated) {
						...
						if (ctrl.taggingTokens.isActivated) {
							for (
								var i = 0;
								i < ctrl.taggingTokens.tokens.length;
								i++
							) {
								if (
			...
			ctrl.searchInput.on('paste', function(e) {
				...
				if (window.clipboardData && window.clipboardData.getData) {
					...
					data = (e.originalEvent || e).clipboardData.getData(
						'text/plain'
					);
				}
				...
				if (data && data.length > 0) {
					...
					if (ctrl.taggingTokens.isActivated) {
						...
						for (
							...
							var separator =
								KEY.toSeparator(ctrl.taggingTokens.tokens[i]) ||
								ctrl.taggingTokens.tokens[i];
						...
						angular.forEach(items, function(item) {
							var newItem = ctrl.tagging.fct
								? ctrl.tagging.fct(item)
								: item;
							if (newItem) {
			...
			function _ensureHighlightVisible() {
				...
				var choices = container.querySelectorAll(
					'.ui-select-choices-row'
				);
				if (choices.length < 1) {
					throw uiSelectMinErr(
		...
		function(
			...
			return {
				...
				templateUrl: function(tElement, tAttrs) {
					...
					return (
						theme +
						(angular.isDefined(tAttrs.multiple)
				...
				compile: function(tElement, tAttrs) {
					...
					if (angular.isDefined(tAttrs.multiple))
						tElement
							.append('<ui-select-multiple/>')
							.removeAttr('multiple');
					else tElement.append('<ui-select-single/>');

					if (tAttrs.inputId)
						tElement.querySelectorAll(
							'input.ui-select-search'
						)[0].id =
							tAttrs.inputId;
					...
					return function(
						...
						scope.$watch('removeSelected', function() {
							var removeSelected = scope.$eval(
								attrs.removeSelected
							);
						...
						attrs.$observe('resetSearchInput', function() {
							...
							var resetSearchInput = scope.$eval(
								attrs.resetSearchInput
							);
							$select.resetSearchInput =
						...
						attrs.$observe('spinnerEnabled', function() {
							// $eval() is needed otherwise we get a string instead of a boolean
							var spinnerEnabled = scope.$eval(
								attrs.spinnerEnabled
							);
							$select.spinnerEnabled =
								spinnerEnabled !== undefined
									? spinnerEnabled
						...
						function onDocumentClick(e) {
							...
							if (window.jQuery) {
								...
								contains = window.jQuery.contains(
									element[0],
									e.target
								);
							} else {
							...
							if (!contains && !$select.clickTriggeredSelect) {
								...
								if (!$select.skipFocusser) {
									...
									var targetController = angular
										.element(e.target)
										.controller('uiSelect'); //To check if target is other ui-select
									skipFocusser =
										targetController &&
										targetController !== $select; //To check if target is other ui-select
									if (!skipFocusser)
										skipFocusser = ~focusableControls.indexOf(
						...
						transcludeFn(scope, function(clone) {
							...
							var transcluded = angular
								.element('<div>')
								.append(clone);

							var transcludedMatch = transcluded.querySelectorAll(
								'.ui-select-match'
							);
							...
							element
								.querySelectorAll('.ui-select-match')
								.replaceWith(transcludedMatch);

							var transcludedChoices = transcluded.querySelectorAll(
								'.ui-select-choices'
							);
							transcludedChoices.removeAttr('ui-select-choices'); //To avoid loop in case directive as attr
							transcludedChoices.removeAttr(
							...
							element
								.querySelectorAll('.ui-select-choices')
								.replaceWith(transcludedChoices);

							var transcludedNoChoice = transcluded.querySelectorAll(
								'.ui-select-no-choice'
							);
							transcludedNoChoice.removeAttr(
								'ui-select-no-choice'
							...
							if (transcludedNoChoice.length == 1) {
								element
									.querySelectorAll('.ui-select-no-choice')
									.replaceWith(transcludedNoChoice);
							}
						});
						...
						var setDropdownPosUp = function(
							offset,
							offsetDropdown
						) {
							offset = offset || uisOffset(element);
							offsetDropdown =
								offsetDropdown || uisOffset(dropdown);
						...
						var setDropdownPosDown = function(
							offset,
							offsetDropdown
						) {
						...
						scope.calculateDropdownPos = function() {
							if ($select.open) {
								dropdown = angular
									.element(element)
									.querySelectorAll('.ui-select-dropdown');
		...
		function(uiSelectConfig) {
			return {
				...
				templateUrl: function(tElement) {
					...
					// Gets theme attribute from parent (ui-select)
					var theme =
						getAttribute(parent, 'theme') || uiSelectConfig.theme;
					var multi = angular.isDefined(
						getAttribute(parent, 'multiple')
					);

					return (
						theme +
						(multi ? '/match-multiple.tpl.html' : '/match.tpl.html')
					);
				},
						...
						ctrl.removeChoice = function(index) {
							...
							$timeout(function() {
								$select.onRemoveCallback($scope, {
									$item: removedChoice,
									$model: $select.parserResult.modelMapper(
										$scope,
										locals
									)
								});
				...
				link: function(scope, element, attrs, ctrls) {
					...
					ngModel.$parsers.unshift(function() {
						...
						for (var j = $select.selected.length - 1; j >= 0; j--) {
							locals = {};
							locals[$select.parserResult.itemName] =
								$select.selected[j];
							result = $select.parserResult.modelMapper(
								scope,
								locals
							);
					...
					ngModel.$formatters.unshift(function(inputValue) {
						var data =
								$select.parserResult &&
								$select.parserResult.source(scope, {
									$select: { search: '' }
								}), //Overwrite $search
							locals = {},
							result;
						...
						var checkFnMultiple = function(list, value) {
							...
							for (var p = list.length - 1; p >= 0; p--) {
								...
								if ($select.parserResult.trackByExp) {
									var propsItemNameMatches = /(\w*)\./.exec(
					...
					ngModel.$render = function() {
						...
						if (!angular.isArray(ngModel.$viewValue)) {
							...
							if (
								angular.isUndefined(ngModel.$viewValue) ||
					...
					scope.$watch('$select.disabled', function(
						newValue,
						oldValue
					) {
					...
					function _handleMatchSelection(key) {
						var caretPosition = _getCaretPosition(
								$select.searchInput[0]
							),
						...
						if (
							caretPosition > 0 ||
							($select.search.length && key == KEY.RIGHT)
						)
							return false;
						...
						function getNewActiveMatchIndex() {
							switch (key) {
								...
								case KEY.RIGHT:
									// Open drop-down
									if (
										!~$selectMultiple.activeMatchIndex ||
										curr === last
									) {
										$select.activate();
										return false;
						...
						if (!$select.selected.length || newIndex === false)
							...
							$selectMultiple.activeMatchIndex = Math.min(
								last,
								Math.max(first, newIndex)
							);
					...
					$select.searchInput.on('keyup', function(e) {
						if (!KEY.isVerticalMovement(e.which)) {
							scope.$evalAsync(function() {
								$select.activeIndex =
									$select.taggingLabel === false ? -1 : 0;
							});
						}
						// Push a "create new" item into array if there is a search string
						if (
							$select.tagging.isActivated &&
							$select.search.length > 0
						) {
							...
							if ($select.tagging.fct !== undefined) {
								...
								newItem =
									$select.search + ' ' + $select.taggingLabel;
								if (
									_findApproxDupe(
										$select.selected,
										$select.search
								...
								if (
									_findCaseInsensitiveDupe(
										stashArr.concat($select.selected)
									)
								) {
			...
			return {
				...
				link: function(scope, element, attrs, ctrls) {
					...
					var removeClass = function(className) {
						angular.forEach(
							$select.$element.querySelectorAll('.' + className),
							function(el) {
				...
				return {
					...
					repeatExpression: function(grouped) {
						var expression =
							this.itemName +
							' in ' +
							(grouped ? '$group.items' : '$select.items');
						if (this.trackByExp) {
...
angular.module('ui.select').run([
	...
	function($templateCache) {
		...
		$templateCache.put(
			...
			'<div class="ui-select-match" ng-hide="$select.open && $select.searchEnabled" ng-disabled="$select.disabled"><span tabindex="-1" class="btn btn-default form-control ui-select-toggle" aria-label="{{ $select.baseTitle }} activate" ng-disabled="$select.disabled" ng-click="$select.activate()" style="outline: 0;"><span ng-show="$select.isEmpty()" class="ui-select-placeholder text-muted">{{$select.placeholder}}</span> <span ng-hide="$select.isEmpty()" class="ui-select-match-text pull-left" ng-class="{\'ui-select-allow-clear\': $select.allowClear && !$select.isEmpty()}" ng-transclude=""></span> <i class="caret pull-right"></i> <a ng-show="$select.allowClear && !$select.isEmpty() && ($select.disabled !== true)" aria-label="{{ $select.baseTitle }} clear" style="margin-right: 10px" ng-click="$select.clear($event)" class="btn btn-xs btn-link pull-right"><i class="glyphicon iconfont-remove" aria-hidden="true"></i></a></span></div>'
		);
		$templateCache.put(
