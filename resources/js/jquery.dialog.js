...
(function(factory) {
	...
	$.fn.dialog = function(parameter, getApi) {
		...
		return this.each(function() {
			//全局变量
			var $this = $('<div class="dialog"></div>');
			var $children = options.content ? $(options.content) : $this.html(); //内容区域
			var $container = $(
				'<div class="' + options.prefix + '-container"></div>'
			);
			var $overlay = $();
			var $close = $(
			...
			var $wg_head = $('<div class="' + options.prefix + '-head"></div>')
				.append($title)
				.append($close);
			var $wg_body = $(
				'<div class="' + options.prefix + '-body"></div>'
			).append($children);
			...
			if (options.isModel) {
				$overlay = $(
					'<div class="' + options.prefix + '-overlay"></div>'
				)
					.css({
						position: _position,
						'z-index': '998',
			...
			_api.resize = function() {
				$container.css({
					left:
						($window.width() - $container.outerWidth()) / 2 + 'px',
					top:
						($window.height() - $container.outerHeight()) / 2 + 'px'
				});
			};
			//设置标题
